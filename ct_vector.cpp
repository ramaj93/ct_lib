...
vector::vector(double _x,double _y,int _dim,double _z){
    if(_dim!=2){
        x=_x;
        y=_y;
        z=_z;
...
void vector::set(char com ,double value=0,bool _err=false){
    if(com =='x') x = value;
...
vector vector::Cross(double _x,double _y,double _z=0){
    vector cp(0,0,0,0);
        if(dim == 2){
            cp.set('d',(x*_y)-(y*_x));
            cp.set('e',false);
...
double vector::Dot(double _x,double _y,double _z=0){
    return x*_x + y*_y + z*_z;
}
double vector::Dot(vector _vc){
    return x* _vc.get('x')+y*_vc.get('y')+z*_vc.get('z');
}

double vector::Magnitude(double _x=0,double _y=0,double _z=0){
    if (_x==0&&_y==0&&_z==0){
...
double vector::Magnitude(vector _vc){
...
void vector::print(char com){
    if(com == 'v') {
    	//todo separate display of 2dim vs 3dim vectors
    	switch(dim){
	    	case 2:
